...
class CreateRegion extends React.Component 
{
    constructor(props)
    {
        ...
        {
            let id=parseInt(_id);
                axios.post(apiURL.Regions.get,{id:id}).then(result => {
    ...
    save = () => {
        ...
        if(edit)
        {
            url = apiURL.Regions.edit;
        }
        else
        {
            url = apiURL.Regions.create;
        }
        axios.post(url,data).then((result) => {
            window.location = appURL.Regions.list;
        });
    };
    render()
    {
        ...
        return (
            <Grid container direction="column" spacing={2} >
                <TextField value={this.state.data.Region} label="Region Name" onChange={this.onChangeRegion} variant="filled"></TextField>
